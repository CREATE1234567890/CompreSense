...
@main.command('advanced-compression')
@click.argument('input_file', type=click.File('r'))
@click.argument('output_directory', type=click.Path(file_okay=False, exists=True))
@click.option('-m', '--model-path', default=None, help='Path to trained XGBoost model (.json)')
@click.option('-o', '--outfile-name', help='Output file name', default='advanced_compression.jsonl', show_default=True)
@click.option('-g', '--gold-file', default=None, type=click.Path(exists=True, dir_okay=False),
              help='Optional gold JSONL path for immediate evaluation')
@click.option('--threshold', default=0.5, type=float, show_default=True,
              help='Classification threshold used for ACC/F1 metrics')
@click.option('--tune-threshold-for', type=click.Choice(['none', 'acc', 'f1']), default='none', show_default=True,
              help='Optionally tune threshold on the provided gold file')
def advanced_compression(input_file, output_directory, model_path, outfile_name, gold_file, threshold,
                         tune_threshold_for):
    """
    Improved compression detector: PPMd/zlib multi-feature + XGBoost.
    """
    from .advanced_compression import AdvancedCompressionDetector
    from .evaluation import evaluate_prediction_file

    detector = AdvancedCompressionDetector(model_path=model_path)
    detect(detector, input_file, output_directory, outfile_name)

    if gold_file:
        pred_file = os.path.join(output_directory, outfile_name)
        metrics = evaluate_prediction_file(
            pred_file,
            gold_file,
            threshold=threshold,
            tune_threshold_for=tune_threshold_for,
        )
        click.echo(
            f"Eval | n={metrics['n']} | auc={metrics['auc']:.6f} | "
            f"acc={metrics['acc']:.6f} | precision={metrics['precision']:.6f} | "
            f"recall={metrics['recall']:.6f} | f1={metrics['f1']:.6f} | "
            f"tn={metrics['tn']} fp={metrics['fp']} fn={metrics['fn']} tp={metrics['tp']} | "
            f"threshold={metrics['threshold']:.3f} | threshold_mode={metrics['threshold_mode']}"
        )


@main.command('train-advanced-compression')
@click.argument('train_jsonl', type=click.Path(exists=True, dir_okay=False))
@click.option('-m', '--model-out', default=None, help='Model output path (.json)')
@click.option('--test-size', default=0.2, type=float, show_default=True)
@click.option('--seed', default=42, type=int, show_default=True)
@click.option('--device', default='cpu', show_default=True,
              help='XGBoost device: cpu, cuda, or cuda:<index>')
@click.option('--class-weight-mode', type=click.Choice(['none', 'balanced']), default='none', show_default=True,
              help='Class reweighting strategy for imbalanced labels')
@click.option('--class0-weight-multiplier', default=1.0, type=float, show_default=True,
              help='Multiplier applied on class0/class1 ratio when class-weight-mode=balanced')
def train_advanced_compression(train_jsonl, model_out, test_size, seed, device,
                               class_weight_mode, class0_weight_multiplier):
    """
    Train advanced compression detector from PAN JSONL data.
    """
    from .advanced_compression import AdvancedCompressionDetector

    metrics = AdvancedCompressionDetector.train(
        ...
        class0_weight_multiplier=class0_weight_multiplier,
    )
    msg = (
        f"Saved model to {metrics['model_out']} | "
        f"val_auc={metrics['val_auc']:.4f} | "
        f"val_acc={metrics['val_acc']:.4f} | "
        f"val_f1={metrics['val_f1']:.4f} | "
        f"n_samples={metrics['n_samples']} | "
        f"class0={metrics['n_class_0']} | "
        f"class1={metrics['n_class_1']}"
    )
    if 'scale_pos_weight' in metrics:
        msg += f" | scale_pos_weight={metrics['scale_pos_weight']:.4f}"
    click.echo(msg)
