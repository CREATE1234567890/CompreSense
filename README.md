...
Commands:
  ...
  advanced-compression  Improved compression detector (PPMd/zlib + XGBoost).
  train-advanced-compression  Train advanced compression detector model.
...
## Advanced Compression Detector

Train a model from PAN training data:

```
pan25-baseline train-advanced-compression pan25-generative-ai-detection-task1-train/train.jsonl
```

Train with class reweighting and GPU:

```
CUDA_VISIBLE_DEVICES=0 pan25-baseline train-advanced-compression \
  pan25-generative-ai-detection-task1-train/train.jsonl \
  --device cuda:0 --class-weight-mode balanced --class0-weight-multiplier 1.3
```

Run inference with the trained model (default model path: `pan25_genai_baselines/compression_xgb_model.json`):

```
pan25-baseline advanced-compression INPUT_FILE OUTPUT_DIRECTORY
```

Or specify a custom model file:

```
pan25-baseline advanced-compression -m /path/to/compression_xgb_model.json INPUT_FILE OUTPUT_DIRECTORY
```

Run inference and print metrics immediately (when gold labels are available):

```
pan25-baseline advanced-compression INPUT_FILE OUTPUT_DIRECTORY \
  -g pan25-generative-ai-detection-task1-train/val.jsonl \
  --tune-threshold-for acc
```
